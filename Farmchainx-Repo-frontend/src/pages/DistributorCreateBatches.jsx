...
const DistributorCreateBatches = ({ crops }) => {
  ...
  const getDistributorId = () => localStorage.getItem('userId');
  ...
  const fetchBatches = async () => {
    try {
      ...
      if (!distributorId) {
        throw new Error('Distributor ID not found. Please login again.');
      }

      // Fetch batches created by this distributor
      const response = await fetch(`${BASE_URL}/batches/user/${distributorId}`, {
        headers: getHeaders()
      });
  ...
  // Fetch available transactions (paid and delivered)
  const fetchAvailableTransactions = async () => {
    try {
      const distributorId = getDistributorId();
      if (!distributorId) return;

      // Fetch transactions where distributor is the receiver (purchases from farmers)
      const response = await fetch(`${BASE_URL}/transactions/to/${localStorage.getItem("userId")}`, {
        headers: getHeaders()
      });
  ...
  }, [batches, searchTerm, cropFilter, statusFilter, dateRange, sortBy, sortOrder]);

  // Open create batch modal with available transactions
  const handleOpenCreateBatch = () => {
    if (availableTransactions.length === 0) {
      setError('No available transactions found. Please purchase crops from farmers first.');
      return;
    }
    setShowCreateBatchModal(true);
  };

  // Select transaction for batch creation
  ...
  const handleCreateBatchFromTransaction = async (e) => {
    ...
    try {
      const distributorId = getDistributorId();
      if (!distributorId) throw new Error('Distributor ID not found. Please login again.');
      ...
      // Prepare batch data with price fields
      const batchData = {
      ...
      const response = await fetch(`${BASE_URL}/batches/distributor/${distributorId}/crop/${formData.cropId}`, {
        method: 'POST',
        headers: getHeaders(),
  ...
  const generateBatchNumber = () => {
    ...
    return `DIST-BATCH-${timestamp}-${random}`;
  };
      ...
      {filteredBatches.length === 0 ? (
        <div style={styles.emptyState}>
          <div style={styles.emptyIcon}>
            <Package size={48} style={{color: '#007b91'}} />
          </div>
                  ...
                  {availableTransactions.length === 0 ? (
                    <div style={styles.emptyTransactionState}>
                      ...
                      <p style={styles.emptyTransactionText}>
                        No available transactions found. Please purchase crops from farmers first.
                      </p>
                    </div>
                  ) : (
                    availableTransactions.map((transaction) => (
                      ...
                      >
                        ...
                        <div style={styles.transactionDetails}>
                          ...
                          <div style={styles.transactionDetail}>
                            <span style={styles.detailLabel}>From Farmer:</span>
                ...
                <>
                  ...
                  <div style={styles.formGrid}>
                    <div style={styles.formGroup}>
                      ...
                      <input
                        ...
                        style={styles.input}
                        required
                        placeholder="e.g., DIST-BATCH-001"
                      />
                    </div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setFormData({
                            ...
                            // Auto-set available quantity to match total quantity when total changes
                    ...
                    <div style={styles.formGroup}>
                      ...
                      <input
                        ...
                        style={styles.input}
                        required
                        placeholder="Enter your selling price per kg"
                      />
                      <small style={{color: '#6b7280', fontSize: '12px'}}>
                        Total price will be auto-calculated: ₹{formData.price || '0.00'}
                      </small>
                    </div>
                    ...
                    <div style={styles.formGroup}>
                      ...
                      <input
                        ...
                        style={{...styles.input, background: '#f9fafb'}}
                        disabled
                        placeholder="Auto-calculated from price per kg and available quantity"
                      />
                      <small style={{color: '#6b7280', fontSize: '12px'}}>
                  ...
                  <div style={styles.priceCalculation}>
                    <div style={styles.calculationBox}>
                      <h5 style={styles.calculationTitle}>Price Calculation</h5>
                      <div style={styles.calculationDetail}>
                        <span>Price per kg: ₹{formData.pricePerUnit || '0.00'}</span>
                        <span>×</span>
                        <span>Available Quantity: {formData.availableQuantity || '0'} kg</span>
                        <span>=</span>
                        <span style={styles.calculationResult}>Total Price: ₹{formData.price || '0.00'}</span>
                      </div>
                    </div>
                  </div>
            ...
            <form onSubmit={handleUpdateBatch} style={styles.modalContent}>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setFormData({
                        ...formData, 
                        quantity: newQuantity,
                        // Auto-adjust available quantity if it exceeds new total
                        availableQuantity: Math.min(parseFloat(formData.availableQuantity) || 0, parseFloat(newQuantity) || 0).toString()
                      });
                    }}
                    style={styles.input}
                    required
              ...
              <div style={styles.priceCalculation}>
                <div style={styles.calculationBox}>
                  <h5 style={styles.calculationTitle}>Price Calculation</h5>
                  <div style={styles.calculationDetail}>
                    <span>Price per {formData.unit}: ₹{formData.pricePerUnit || '0.00'}</span>
                    <span>×</span>
                    <span>Available Quantity: {formData.availableQuantity || '0'} {formData.unit}</span>
                    <span>=</span>
                    <span style={styles.calculationResult}>Total Price: ₹{formData.price || '0.00'}</span>
                  </div>
                </div>
            ...
            <form onSubmit={handleUpdateStatus} style={styles.modalContent}>
              <div style={styles.batchInfo}>
                ...
                <div style={styles.priceInfo}>
                  <span>Price: ₹{selectedBatch.price} (₹{selectedBatch.pricePerUnit} per {selectedBatch.unit})</span>
                </div>
              </div>
            ...
            <div style={styles.qrContent}>
              <div style={styles.qrInfo}>
                ...
                <div style={styles.qrDetails}>
                  ...
                  <span>Price: ₹{selectedBatch.price} (₹{selectedBatch.pricePerUnit}/{selectedBatch.unit})</span>
                </div>
              </div>
...
// Styles object with proper font sizes and responsive design
const styles = {
  tabContentInner: {
    padding: '24px',
    maxWidth: '1450px',
...
export default DistributorCreateBatches;